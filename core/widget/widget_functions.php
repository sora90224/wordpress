...
function sir_community_widgets_init() {
    ...
    register_sidebar( array(
        'name'          => __( '메인 1 영역', 'sir-furniture' ),
        'id'            => 'main-head-latest',
        'description' 		=> __('사이트 메인에만 적용됩니다.', 'sir-furniture' ),
    ...
    for ( $i = 1; $i <= intval( $main_latest_widget_regions ); $i++ ) {
        register_sidebar( array(
            'name'          => sprintf( __( '일반 최신글 %d', 'sir-furniture' ), $i ),
            'id'            => sprintf( 'main-latest-%d', $i ),
            'description' 		=> sprintf( __( '위젯 최신글 영역 %d.', 'sir-furniture' ), $i ),
            'before_widget' => '',
            'after_widget'  => '',
            'before_title'  => '<h2>',
            'after_title'   => '</h2>',
